...
def get_template_post(platform, tone, topic):
    ...
    # Normalize topic for hashtag
    topic_clean = topic.replace(' ', '').replace('-', '').lower()
    
    if platform in TEMPLATES and tone in TEMPLATES[platform]:
        template = TEMPLATES[platform][tone]
        # Replace topic in template (only full words, not substrings)
        # Use word boundaries or specific patterns
        result = template
        # Simple replacements for common topics
        if 'автоматизация' in topic.lower():
            result = result.replace('автоматизация', topic)
        elif 'бизнес' in topic.lower():
            result = result.replace('бизнеса', topic + 'а').replace('бизнес', topic)
        else:
            # Generic replacement
            result = template.replace('автоматизация бизнеса', topic)
        return result
